...
def k_struct(r):
    return (
        ...
        r["datven"],r["preexe"],r["indopc"],r["ptoexe"]
    )

def stats(r):
    return {n:r[n] for n in ("totneg","quatot","voltot","preab","premax","premin","premed","preult")}

def main(path):
    raw=Path(path).read_bytes()
    sha=hashlib.sha256(raw).hexdigest()
    gp,gt=defaultdict(list),defaultdict(list)
    records=0
    for raw_line in raw.splitlines():
        ...
        if len(line)<245 or line[:2]!="01":
            continue
        records+=1
        r={n:get(line,n) for n in FIELDS}
        if r["tpmerc"]=="030":
            gp[k_partial(r)].append(r)
            gt[k_struct(r)].append(r)
    ...
    out={
        ...
        "term_full_k4_groups":len(gt),
        "term_full_k4_collisions":len(fc),
        "term_full_k4_collisions_with_stat_difference":sum(x["distinct_stat_profiles"]>1 for x in fc),
        "partial_examples":pm[:200],
        "full_collision_examples":fc[:50],
        "k4_fields":["dt","codbdi","codneg","tpmerc","codisi","dimes","especi","prazot","datven","preexe","indopc","ptoexe"],
